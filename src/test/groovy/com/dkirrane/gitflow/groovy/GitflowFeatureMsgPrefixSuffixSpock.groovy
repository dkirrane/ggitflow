...
@Stepwise
@Slf4j
class GitflowFeatureMsgPrefixSuffixSpock extends Specification {
    @Shared @ClassRule TemporaryFolder tempFolder = new TemporaryFolder()
    @Shared def repoDir
    @Shared def init
    static final FEATURE_BRN_NAME = 'JIRA-123'
    static final MSG_PREFIX = "Message Prefix"
    static final MSG_SUFFIX = "Message Suffix"
    ...
    def "Gitflow feature start"() {
        ...
        def feature = new GitflowFeature(init:init, squash:true, msgPrefix:MSG_PREFIX, msgSuffix:MSG_SUFFIX)
    ...
    def "Gitflow feature finish"() {
        ...
        and: "the feature branch commits should be and then merged into develop"
        def squashMessage = init.executeLocal("git log -2 --pretty=%B")
        assert squashMessage.contains("${MSG_PREFIX} Squashing branch '${featureBranch}' into develop ${MSG_SUFFIX}")
        def mergeMessage = init.executeLocal("git log -1 --pretty=%B")
        assert mergeMessage.contains("${MSG_PREFIX} Merge branch '${featureBranch}' into develop ${MSG_SUFFIX}")
    }
